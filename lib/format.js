...
export function articleLink( data, headTag = 'h2' ) {
  ...
  return `
    <article>
      <a href="${ data.link }">
        ${ data.hero ? `<img src="${ tacs.root }assets/img/${ data.hero }" class="back">` : '' }
        <div>
          ${ data.date ? `<p class="pubdate"><time datetime="${ dateISO( data.date ) }"><img src="${ tacs.root }assets/img/icons/vl-meta-ic-3.1.svg"> ${ dateHuman( data.date ) }</time></p>` : '' }
